...
static const char *TAG = "modbus_spy.sensor";

void ModbusSpy::setup()
{
  xTaskCreatePinnedToCore(ModbusSpy::read_loop_task,
                              "read_task", // name
...
void ModbusSpy::read_loop_task(void* params) {
  ModbusSpy *modbus_sniffer = reinterpret_cast<ModbusSpy*>(params);
  while (true) {
...
{
  ESP_LOGCONFIG(TAG, "ModbusSpy sensor");
